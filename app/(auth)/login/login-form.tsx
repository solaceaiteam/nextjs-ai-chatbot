...
export function LoginForm() {
  ...
  useEffect(() => {
    ...
    if (error && !isProcessing && !isSuccessful) {
      toast({
        type: 'error',
        description:
  ...
  }, [error, isProcessing, isSuccessful]);
  ...
  useEffect(() => {
    ...
    if (state.status === 'in_progress') {
      ...
      if (state.status === 'success' && !isSuccessful) {
        ...
        // Redirect immediately, let middleware/session handle auth
  ...
  const handleGoogleSignIn = () => {
    if (isProcessing) return;
    // Always redirect to main site after login
    import('next-auth/react').then(({ signIn }) =>
      signIn('google', {
        callbackUrl: 'https://solaceai.xyz',
        redirect: true,
      }),
    );
  };
  ...
  return (
    <>
      ...
      >
        ...
        {isProcessing ? 'Signing in...' : 'Log in with Google'}
      </button>
    </>
  );
}
